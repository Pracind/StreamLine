...
def count_keyword_hits_per_chunk(logger=None):
    ...
    logger.info("TEXT SCORING: started")
    logger.info("KEYWORDS_PATH: %s", KEYWORDS_PATH)
    logger.info("KEYWORDS_EXISTS: %s", KEYWORDS_PATH.exists())
    ...
    try:
        ...
        logger.info("TEXT SCORING: completed successfully")
        return all_features

    except Exception:
        logger.exception("TEXT SCORING FAILED")
        raise
